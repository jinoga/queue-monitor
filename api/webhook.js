...
async function handleEvent(event) {
    ...
    if (text.startsWith('ติดตามคิว')) {
        ...
        // --- ส่วนที่ 1: เช็คโควต้า (คงเดิมตามที่คุณต้องการ) ---
        try {
        ...
        // --- ส่วนที่ 2 (Logic หลัก): ดึงคิวล่าสุด "เฉพาะหมวดเดียวกัน" ---
        // ป้องกันการเอาคิว 1010 มาเทียบกับ 7020 แล้วบอกว่ารอ 6000 คิว
        ...
        try {
            ...
            // สูตร: ปัดเศษหลักพันลง * 1000 (เช่น 7020 -> 7000 ถึง 8000)
            const seriesStart = Math.floor(targetQueue / 1000) * 1000;
            const seriesEnd = seriesStart + 1000;

            const { data: statusData } = await supabase
                .from('queue_snapshots') // อ่านจากตารางที่ Node.js บันทึก
                .select('current_queue, current_counter')
                // 🔎 กรอง: เอาเฉพาะคิวที่อยู่ในหมวดหลักพันเดียวกัน
        ...
        try {
            ...
            // --- ส่วนที่ 3: แสดงผล (ข้อความเดิมของคุณ) ---
            let replyText = `✅ เริ่มติดตามคิว ${targetQueue} ให้คุณแล้ว`;

            // ถ้าเจอคิวในหมวดเดียวกัน (currentQueue > 0) ถึงจะคำนวณ diff
            // ถ้าไม่เจอ (เป็น 0) แสดงว่ายังไม่มีการเรียกคิวหมวดนี้ ให้ข้ามไปแสดงข้อความรอ
            if (currentQueue > 0) {
                ...
                if (diff < 0) {
                    // diff ติดลบ = เลยคิวแล้ว (ในหมวดเดียวกัน)
                ...
                // กรณีไม่เจอข้อมูลคิวในหมวดนี้เลย (เช่น มีแต่ 1010 แต่ตาม 7020)
                // ใช้ข้อความมาตรฐานเดิม ไม่แจ้งว่าเลยคิว
