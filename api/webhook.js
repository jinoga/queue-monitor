...
async function handleEvent(event) {
    ...
    if (text.startsWith('ติดตามคิว')) {
        ...
        // ==================================================================
        // 🔴 ส่วนที่ 2 (แก้ใหม่หมด): ดึงมาเยอะๆ แล้วกรองด้วย JS (ชัวร์ที่สุด)
        // ==================================================================
        let currentQueue = 0;
        let currentCounter = '-'; 

        try {
            // คำนวณช่วงหมวดคิว (4012 -> 4000 ถึง 5000)
            const seriesStart = Math.floor(targetQueue / 1000) * 1000;
            const seriesEnd = seriesStart + 1000;

            // 1. ดึงข้อมูลล่าสุดมา 50 รายการ (เผื่อมีคิวหลายช่องวิ่งสลับกัน)
            const { data: snapshots } = await supabase
                .from('queue_snapshots') 
                .select('current_queue, current_counter')
                .order('created_at', { ascending: false })
                .limit(50); // ดึงมาเผื่อเลือก

            if (snapshots && snapshots.length > 0) {
                // 2. ใช้ JavaScript วนหาตัวแรก ที่เลขคิว "ตรงกับหมวด" ของเรา
                // วิธีนี้แก้ปัญหาเรื่อง Database Query เพี้ยนได้ 100%
        ...
        try {
            ...
            // --- ส่วนที่ 3: แสดงผล (ข้อความเดิมครบถ้วน) ---
